...
func NewAdaptiveConcurrencyManager() *AdaptiveConcurrencyManager {
	return &AdaptiveConcurrencyManager{
		...
		minLimit:            2,                               // 최소 2개
		maxLimit:            20,                              // 최대 20개
		responseTimeWindow:  make([]time.Duration, 0, 50),
		windowSize:          50,
		adjustmentThreshold: 500 * time.Millisecond, // 500ms 이상이면 감소 고려
		decreaseThreshold:   1 * time.Second,        // 1초 이상이면 즉시 감소
		adjustmentCooldown:  5 * time.Second,        // 5초마다 조정
...
func (acm *AdaptiveConcurrencyManager) RecordResponseTime(responseTime time.Duration) {
	...
	if len(acm.responseTimeWindow) >= 10 && time.Since(acm.lastAdjustment) > acm.adjustmentCooldown {
...
func (acm *AdaptiveConcurrencyManager) calculateP95ResponseTime() time.Duration {
	...
	cutoff := maxTime * 8 / 10 // 대략적인 95 퍼센타일
